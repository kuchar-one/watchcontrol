...
class WatchClient:
    ...
    def __init__(self, address: str):
        ...
        self._bc_buffer = bytearray()
        self._bc_expected_len = 0
    ...
    def _notification_handler(self, _sender, data: bytearray):
        ...
        if raw and raw[0] == 0xBC:
            # BC large-data packets may be fragmented across multiple notifications.
            # Buffer until the full payload (per the length field in the header) arrives.
            if not self._bc_buffer and len(raw) >= 4:
                payload_len = raw[2] | (raw[3] << 8)
                self._bc_expected_len = 6 + payload_len
            self._bc_buffer.extend(raw)

            if self._bc_expected_len > 0 and len(self._bc_buffer) >= self._bc_expected_len:
                complete = bytes(self._bc_buffer[:self._bc_expected_len])
                self._bc_buffer = bytearray()
                self._bc_expected_len = 0
                packet = protocol.Packet.from_bytes(complete)
                if packet:
                    logger.debug(f"Received BC Packet: ACT {packet.action_id} len={len(packet.payload)}")
                    self._response_queue.put_nowait(packet)
            return

        packet = protocol.Packet.from_bytes(raw)
        if packet:
            logger.debug(f"Received Packet: CMD {packet.cmd_id} (Large=False)")
            self._response_queue.put_nowait(packet)
    ...
    async def sync_sleep(self, day_offset: int = 0, debug: bool = False):
        """
        Sync sleep data for the given day offset using the high-speed BC channel.
        day_offset: 0 = today, 1 = yesterday, etc.

        Response payload (Action 39 / "New Sleep" protocol):
          [0] day_offset echo
          [1:3] start_minutes LE  – sleep start, minutes since midnight
          [3:5] end_minutes LE    – sleep end, minutes since midnight
          [5+]  (type, duration_minutes) pairs

        Sleep type mapping (confirmed from APK):
          2 = Light Sleep, 3 = Deep Sleep, 4 = REM, 5 = Awake
        """
        print(f"Requesting sleep data for day offset {day_offset}...")
        await self._send(protocol.Packet(
            is_large_data=True,
            action_id=protocol.ActionID.SLEEP_SYNC,
            payload=bytes([day_offset & 0xFF, 0x01]),
        ))

        try:
            packet = await self._wait_for_packet(action_id=protocol.ActionID.SLEEP_SYNC, timeout=10.0)
        except asyncio.TimeoutError:
            print("Sleep sync timed out (no response from watch).")
            return None

        if debug:
            print(f"DEBUG: raw payload ({len(packet.payload)} bytes): {packet.payload.hex()}")
        ...
        if len(packet.payload) < 7:
            print(f"Malformed sleep response (only {len(packet.payload)} bytes).")
            return None

        # Payload layout (confirmed by reverse-engineering watch output):
        #   [0]   day_offset echo
        #   [1]   0x00 (padding / high byte of length field)
        #   [2]   data_length – number of bytes from [3] to end
        #   [3:5] start_minutes LE – sleep start, minutes from midnight of sleep date
        #   [5:7] end_minutes LE   – wake time, minutes from midnight of wake date
        #   [7+]  (sleep_type, duration_minutes) pairs
        start_minutes = packet.payload[3] | (packet.payload[4] << 8)
        end_minutes   = packet.payload[5] | (packet.payload[6] << 8)

        segments = []
        idx = 7
        while idx + 1 < len(packet.payload):
            seg_type = packet.payload[idx]
            seg_dur  = packet.payload[idx + 1]
            if seg_type != 0 or seg_dur != 0:
                segments.append({"type": seg_type, "duration": seg_dur})
            idx += 2

        # Wake date = the date the user woke up (day_offset days before today).
        # Sleep typically starts in the evening, so the sleep start date is one day earlier.
        wake_date = datetime.datetime.now().date() - datetime.timedelta(days=day_offset)
        sleep_date = wake_date - datetime.timedelta(days=1) if start_minutes >= 12 * 60 else wake_date

        # When sleep crosses midnight, start > end (both measured from their respective midnights).
