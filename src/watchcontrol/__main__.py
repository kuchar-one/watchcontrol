...
async def handle_command(args):
    if args.command == "scan":
        ...
        async with await get_client(args) as client:
            all_segments = await client.sync_sleep(day_offset=args.days)
            if not all_segments:
                print("No sleep data found.")
                return
            
            print(f"\n--- Sleep Data (Offset: {args.days}) ---")
            current_date = all_segments[0]["date"]
            print(f"Date: {current_date}")
            
            for seg in all_segments:
                hour = (seg["time_index"] * 15) // 60
                minute = (seg["time_index"] * 15) % 60
                time_str = f"{hour:02d}:{minute:02d}"
                
                q_names = [SLEEP_TYPES.get(q, f"UNKNOWN({q})") for q in seg["qualities"] if q != 0]
                if q_names:
                    print(f"[{time_str}] {' -> '.join(q_names)}")
